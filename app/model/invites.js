...
module.exports = app => {
    ...
    if (invite.dataValues.is_accept === 1 && invite._previousDataValues.is_accept === 0 && invite.receiver_id) {
      if (invite.group === 'Projects') {
        ...
        ctx.model.Messages.create({
          ...
          type: 'personal',
          url: `/project-manage/project/${invite.group_id}`,
        });
      }
    }
  });
  ...
  invite.associate = function (models) {
    ...
    // belongsTo和hasOne还是不能乱用,会导致as字段的值拿不到数据,在别的model中有说明
    // invite.hasOne(app.model.User, { foreignKey: 'id', sourceKey: 'actor_id', as: 'actor' });
    invite.belongsTo(app.model.User, { foreignKey: 'actor_id', targetKey: 'id', as: 'actor' });
  };
  return invite;
};
